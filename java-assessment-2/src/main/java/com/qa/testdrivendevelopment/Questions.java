...
public class Questions {
	 ...
	 */
	public int returnPosition(String input, int letter) {

		int position = 0;
		boolean containsLetter = input.indexOf(letter) != -1 ? true : false;

		if (containsLetter == true) {
			input.toLowerCase();
			String newInput = input.replaceAll("\\s+", "");
			position = newInput.indexOf(letter) + 1;
	...
	public String zipped(String input1, String input2) {
		...
		while (j < input2.length()) {
			...
			while (i < input1.length()) {
				words += input1.substring(0, i);
				i++;
			}

			words += input2.substring(0, i);
			j++;
		}
	...
	public String[] removeDuplicates(String[] stringArray) {
		...
		LinkedHashSet<String> removeDupe = new LinkedHashSet<String>(Arrays.asList(stringArray));

		String[] newArray = removeDupe.toArray(new String[removeDupe.size()]);
	...
	public int[] listGen() {
		...
		int num3 = r.nextInt(high - low) + low;
		int num4 = r.nextInt(high - low) + low;
		int num5 = r.nextInt(high - low) + low;
	...
	public int getVowel(String input) {
		 ...
		 * int vowels = 0;
		 * 
		 * for (int i = 0; i < input.length(); i++) { input.toLowerCase(); char vowel =
		 * input.charAt(i);
		 * 
		 * if (vowel == "a" || vowel == "e" || vowel == "i" || vowel == "o" || vowel ==
		 * "u") { vowels++; } } System.out.println(vowels); return vowels; }
		 */
	...
	public boolean wordFinder(String initialWord, String madeString) {
		...
		// make madeSting into an array then check for individual character
		// unless it's only looking for the specific string in order

		boolean containsString = initialWord.indexOf(madeString) != -1 ? true : false;
